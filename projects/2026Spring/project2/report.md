...
- [3. 实验环境](#3-实验环境)
- [4. 软件程序](#4-软件程序)
- [5. 运行脚本与编译参数](#5-运行脚本与编译参数)
- [6. 实验方案设计](#6-实验方案设计)
- [7. 实验过程与关键结果](#7-实验过程与关键结果)
- [8. 跨平台差异](#8-跨平台差异)
- [9. 实验结果](#9-实验结果)
- [10. checksum 的正确性验证](#10-checksum-的正确性验证)
- [11. 终极释放：硬件原生指令集与 Vector API 的对决](#11-终极释放硬件原生指令集与-vector-api-的对决)
- [12. 心得](#12-心得)
- [13. 附录](#13-附录)
...
6. `asm volatile("" ::: "memory")`（编译器屏障）意思是内存可能随时被外界改写，告诉编译器每一次循环都要重新读取，目的是减少“循环被过度优化”带来的假快现象。（具体参见 7.1 节）
7. `__attribute__((noinline))`（禁止内联）用于防止函数被内联展开，保持测试的独立性和稳定性。但是我在后续实验中发现一个违背直觉的现象，去掉 `noinline` 后程序更慢了——也就是说，允许 dot_once 内联后程序性能反而变差了（结果见下图）。我的分析如下：
   ```
...
8. 在分析内联的时候，我使用 `-Rpass-analysis=loop-vectorize` 来观察编译器对哪些循环进行了向量化处理，有用的结果如下：
   ```
...
4. 预热（warmup）阶段：
    - Java 程序在首次运行时，JVM 以解释模式逐行执行字节码，性能极差。随着某段代码被反复执行，HotSpot 虚拟机内置的 C2 即时编译器（JIT）会识别出"热点代码"，将其动态编译为针对当前硬件优化的本地机器码。这个编译过程本身需要时间，且编译完成前后的执行速度可能相差数倍到数十倍。
    - 因此，若不经预热直接计时，第一次测量结果实际上包含了 JIT 编译本身的耗时，严重高估了稳态运行时间，导致 Java 与 C 的对比失去公平性。预热阶段通过提前触发 JIT 编译，确保正式计时时代码已处于完全优化的稳定态。
    - 具体做法：分类型执行 10 轮预热，先让 JIT 进入稳定阶段，再正式计时。
...
- 说明：`-Xms1g -Xmx4g` 固定堆大小用于减少堆扩容抖动，避免大数组OOM；`-server` 在现代 JDK 下通常是默认模式，但显式指定可读性更好。
...
在最初的实验中，发现 `-O1` 在 N 较小时，signed char、short、int 上表现异常快（平均时间为 0 ns），甚至比 `-O2` 和 `-O3` 更快。
<figure>
...
为了理解 `-O1` 反常现象，我搜集各编译优化级别的特点，分析其对点乘性能的影响。
...
然而，当引入 -Ofast 编译选项（其核心机制为隐式开启 -ffast-math，允许编译器放宽精度限制、自由重排浮点指令）后，性能迎来了质的飞跃：float 的单步耗时骤降至 0.332 ns，double 降至 0.395 ns。这一超过 3 倍的性能飙升不仅使其计算速度与int达到了同一数量级，更完美证实了此前浮点计算的性能瓶颈确实在于严格精度规范对 SIMD 向量化指令的“封印”。一旦通过编译选项授予优化特权，现代 CPU 的数据级并行算力将得到彻底释放。
...
![alt text](output_plots/dot_time_compare_median_double.png)
![alt text](output_plots/dot_time_compare_median_float.png)
![alt text](output_plots/dot_time_compare_median_int.png)
![alt text](output_plots/dot_time_compare_median_short.png)
![alt text](output_plots/dot_time_compare_median_signed_char.png)
...
![alt text](windows/output_plots/dot_time_compare_median_double.png)
![alt text](windows/output_plots/dot_time_compare_median_float.png)
![alt text](windows/output_plots/dot_time_compare_median_int.png)
![alt text](windows/output_plots/dot_time_compare_median_short.png)
![alt text](windows/output_plots/dot_time_compare_median_signed_char.png)
...
![alt text](output_plots/win_macos/win_macos_compare_median_double.png)
![alt text](output_plots/win_macos/win_macos_compare_median_float.png)
![alt text](output_plots/win_macos/win_macos_compare_median_int.png)
![alt text](output_plots/win_macos/win_macos_compare_median_short.png)
![alt text](output_plots/win_macos/win_macos_compare_median_signed_char.png)
...
### 9.1 对于浮点数和 short、signed char 类型，C 语言占据统治地位
...
### 9.2 对于 int 类型，Java 反超 C
在整型（`int`）数组的点乘测试中，本实验观察到了一个极其反常识的现象：无论是在 macOS 还是 Windows 平台上，**Java 的执行速度均实质性地超越了开启激进优化 (`-Ofast` / `-O3`) 的 C 语言**（例如在 Windows 平台 `N=10^5` 时，Java 领先 C 语言近 20%）。 

深入底层计算机体系结构分析，此现象并非打破了语言层面的常理，而是揭示了 **JIT（即时编译）与 AOT（提前编译）在指令集调度上的代差**。

C 语言（GCC/Clang）采用 AOT 编译，为了保证分发可执行文件的跨硬件兼容性，默认仅向后兼容生成基础指令集（如 128 位的 SSE2）。这意味着即便开启了 SIMD 向量化，C 语言单次也只能并行处理 4 个 32 位整型。然而如果为 C 也加上 `-march=native`，C 同样能利用 AVX2，此时 C 将重新领先（见第 11 节数据）。
...
Java 逼近标准优化的 C：在标准优化（C-O2/C-O3，未开启破坏性数学优化）的严格安全计算环境下，Java 与 C 的差距被极大缩小。例如，当 N=10^4时，Java 的 double 耗时为 1.33 ns，而 C-O3 为 1.24 ns，两者几乎处于同一水平线。这得益于 JVM 内部强大的 C2 JIT在运行时进行的动态循环展开和热点代码编译。
...
---
## 10. checksum 的正确性验证
为了探究`-O3,-Ofast,-Ofast -march=native` 开启SIMD向量化后，是否会对计算结果的正确性产生影响，我们在每次测试后都对 `checksum` 进行了验证。 

整数类型的 checksum 在所有优化级别下完全一致；浮点类型在 -Ofast 下结果与 -O3 存在极微小的末位差异（在本实验的打印精度下未体现），这与 -ffast-math 允许重排浮点运算顺序的预期行为一致，表明在本实验精度范围内，SIMD 向量化的引入并未破坏计算的正确性。
...
因此，为了探寻硬件算力的绝对极限，本实验最终引入了 C 语言的 **`-march=native`** 编译选项（指令编译器生成专属于当前运行硬件的最高级指令）与 Java 处于孵化期的 **Vector API** (source code见附录)进行对决。

得到的结果如下：
- 在win上，java Vector较普通java double,float明显变快，然而int,short,signed char均稍微变慢；-march=native较单纯-Ofast double,float,int,short,signed_char均明显变快
- 在mac上，java Vector较普通java double float int 均明显变快，short,char稍微变慢；-march=native较单纯-Ofast float,int,short,char均明显变快，double则在n较小的时候快，n大的时候慢。
  
   <figure>
   <img src=image-5.png width = 300 height = 400>
   <figcaption></figcaption>
   </figure>
(windows 平台)

### 11.1 极致算力快照：缓存内计算的绝对天花板 ( `N=10^5` )

为了排除物理内存带宽（Memory Wall）的干扰，纯粹评估 CPU 的计算吞吐量，我们首先选取了 `N=100,000` 的数据规模绘制了峰值性能对比图。此时，所有测试数组均能完全容纳于 CPU 的 L2/L3 高速缓存中。

![alt text](output_plots/showcase/bar_peak_snapshot_mac.png)
![alt text](output_plots/showcase/bar_peak_snapshot_windows.png)

**观察与解析：**
1. **C 语言 AOT 编译的最终解封**：在 Windows 和 macOS 平台上，对比单纯的 `-Ofast`，开启 `-march=native` 后，C 语言在几乎所有数据类型上的性能均得到了进一步的显著提升。这证明了当编译器卸下“兼容旧架构（如 SSE2）”的历史包袱后，能够精准调用宿主机最宽的向量寄存器（如 AVX2 256-bit），从而展现出无与伦比的绝对统治力。
2. **Java Vector API 的双刃剑效应（The Abstraction Overhead）**：
    针对大位宽浮点数 (double, float)：代码中实际应用了 Vector API 进行显式向量化（FloatVector / DoubleVector）。这一举措成功绕过了 JIT 保守的启发式猜测，强制 JVM 直接映射到底层 SIMD 指令，从而在 Mac 和 Windows 上均带来了显著的性能跃升。这证明在处理受限于 IEEE 754 规范的浮点数时，显式向量化编程仍具有不可替代的价值。

    针对整型数据 (int, short, signed char) 的轻微劣化谜题：在数据表现上，这三种类型在 Vector 测试中出现了轻微的变慢。回归代码实现本质，我们在本次扩展实验中并未对这三种整型强制使用 Vector API，而是让其回退执行了与常规版本完全一致的标量循环（dotOnce）。

    既然执行的是同一段标量代码，为何会出现性能衰退？我大胆猜测这是由于 JVM C2 即时编译器的“环境敏感性”。当我们在同一个 Java 程序中引入了 jdk.incubator.vector 模块，并混合执行了高强度的显式向量计算时，底层的方法轮廓信息被改变。这可能导致 C2 编译器在编译标量方法时，采取了不同的寄存器分配策略，亦或是由于包含大体积 Vector 代码导致最终生成的机器码在指令缓存中的物理对齐位置发生了偏移。这种微妙的运行池污染与系统噪声，最终导致相同的逻辑代码出现了约 5%~10% 的性能负面波动。

### 11.2 微架构的物理反噬：Mac 平台上的 AVX-512 降频陷阱

如果说缓存内的算力对决是纯粹逻辑的较量，那么当数据规模急剧放大时，物理法则将开始接管比赛。我们在 macOS 平台（搭载支持 AVX-512 指令集的 Ice Lake 架构 CPU）上观察到了一个极具体系结构价值的“性能反转”现象。

![alt text](output_plots/showcase/line_mac_double_native_vs_ofast.png)


**观察与解析：**
如上图所示，在计算 64 位 `double` 类型的点乘时，`C-Native` 与 `C-Ofast` 的性能曲线在 `N=10^5` 至 `10^6` 之间发生了不可思议的交叉（Crossover）。
* **在小规模数据 (`N <= 10^5`)时**：`C-Native` 凭借 AVX-512 的恐怖吞吐量大幅领先。
* **在大规模数据(`N >= 10^6`)时**：`C-Native` 的性能急剧衰退，不仅丧失了优势，反而比保守的 `C-Ofast` 更加缓慢。

**底层物理机制剖析（Downclocking Penalty）：**
这一反常现象完美印证了微处理器设计中的 **“功耗热节流（Thermal Throttling）”** 机制。当 `-march=native` 强迫 CPU 满载启动 AVX-512 单元执行极其密集的 `double` 乘加运算时，硅片会在瞬间产生巨大的电流消耗和热量。为了防止物理烧毁，Intel 芯片底层的微代码（Microcode）会触发硬件级的保护机制，**强制大幅降低 CPU 的核心运行频率（即 AVX Offset 降频）**。

与此同时，千万级别的数据长度早已击垮了 L3 缓存。此时的 CPU 面临着双重困境：一方面主频被极大地压制，另一方面超宽的 512 位寄存器迅速吸干了内存总线带宽，导致 CPU 大量时钟周期处于等待 RAM 响应的闲置状态。而保守的 `-Ofast` 虽然单次处理数据较少，但成功维持了较高的主频，反而在这场马拉松中取得了最终的胜利。

### 11.3 终极结语

经过本次横跨多语言（C vs Java）、跨硬件架构（Win vs Mac）、跨编译级别与指令集的系统性基准测试，我们得出了关于现代计算性能的核心洞察：

在现代高性能软件工程中，**不存在绝对普适的“性能银弹”**。

极致的程序性能，永远存在于**高级算法逻辑、编译器启发式模型、运行时即时编译状态**，以及**底层硅片微架构的功耗墙与内存带宽**的精密平衡之中。盲目追求最底层的控制权或最激进的指令集（如无脑开启 `-march=native` 或强用 Vector API），在受限于缓存规模或散热边界时，反而会遭到物理法则的无情反噬。

---
## 12. 心得
这次实验的流程大致如下：

从基础实现 -> 发现 C-O3 的异常 -> 解开 -Ofast 浮点封印 -> 惊叹 JIT 的反超 -> 祭出最终武器 Native 与 Vector API -> 最终回归对计算机底层物理限制的敬畏。

一层层盘丝剥茧，从最朴素的算法实现，到编译器优化的微妙差异，再到硬件微架构的物理限制，最后得出性能的最终结论。整个过程就像是一场对计算机体系结构和软件优化的深度探索之旅，每一个阶段都充满了惊喜和启发。

这次实验让我切实体会到：在现代计算机系统中，"写代码"只是第一步，理解编译器、运行时与硬件三者的协同博弈，才是榨取性能的真正关键。

---

## 13. 附录
### 13.1 checksum 验证结果（从左至右分别是 `-O3`、`-Ofast`、`-Ofast -march=native`）
![alt text](image-4.png)

### 13.2 Java Vector API 的实现代码
```
