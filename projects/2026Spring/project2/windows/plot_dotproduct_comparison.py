...
def load_metric_data(metric: str):
    ...
    for level in C_LEVELS:
        if level == "Ofast":
            local_ofast = BASE_DIR / f"results_c({level})_{metric}.csv"
            nested_ofast = BASE_DIR / "ofast_results" / f"results_c({level})_{metric}.csv"
            file_path = local_ofast if local_ofast.exists() else nested_ofast
        else:
