param(
    [int]$Runs = 5,
    [string]$JavaOpts = '-Xms1g -Xmx4g -server --add-modules jdk.incubator.vector'
)
...
$srcFile = 'DotproductVector.java'
$className = 'DotproductVector'
$javacModuleArgs = @('--add-modules', 'jdk.incubator.vector')
...
Write-Host "[INFO] Compiling $srcFile with Vector API ..."
& javac @javacModuleArgs $srcFile
if ($LASTEXITCODE -ne 0) {
    throw 'javac failed.'
}

if ($Runs -eq 1) {
    $outFile = 'results_java_vector.csv'
    Write-Host "[INFO] Running benchmark -> $outFile"
    & java @javaArgList $className | Set-Content -Path $outFile -Encoding utf8
    Write-Host '[DONE] Generated Java Vector files:'
    Get-ChildItem -Name 'results_java_vector*.csv' | Sort-Object
    exit 0
}

Get-ChildItem -Path . -Filter 'results_java_vector_run*.csv' -ErrorAction SilentlyContinue | Remove-Item -Force

$runFiles = @()
foreach ($r in 1..$Runs) {
    $outFile = "results_java_vector_run$r.csv"
...
Set-Content -Path 'results_java_vector_mean.csv' -Value $meanLines -Encoding utf8
Set-Content -Path 'results_java_vector_median.csv' -Value $medianLines -Encoding utf8
Copy-Item -Force "results_java_vector_run$Runs.csv" 'results_java_vector.csv'
