            ...
            for(int j0 = 0; j0<cols2; j0+=block_size) {
                ...
                for(; i <= i_max - 4; i+=4) {
                    ...
                    for( ; j<= j_len - 16;j+=16) {
                        ...
                        for(int k = k0; k < k_max; k++) {
                            // Load 16 elements from packed B
                        ...
                        // Store computed values back to memory
                ...
                for(; i < i_max; i++) {
                    ...
                    int j_res = 0;
                    // AVX2 processing for remaining row elements (1x8 block)
                    for (; j_res <= j_len - 8; j_res += 8) {
                        // Load C values outside the K-loop
                        __m256 vc = _mm256_loadu_ps(cp + j0 + j_res);
                        
                        // Accumulate using registers in the K-loop
                        for(int k = k0; k < k_max; k++) {
                    ...
                    // Scalar fallback for absolute column margins
