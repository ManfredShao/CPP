...
static const size_t k_sizes[] = {10ull, 100ull, 1000ull, 10000ull, 100000ull, 1000000ull, 10000000ull, 100000000ull};
...
static inline int reps_for_n(size_t n)
{
    ...
    // For very large n (1e7/1e8), use explicit low reps to avoid hour-long runs.
    if (n >= 100000000ull)
        return 1;
    if (n >= 10000000ull)
        return 2;

    const size_t target_ops = 20000000ull; // ~20M multiply-adds for n <= 1e6.
...
{
    print_csv_header();
    for (size_t i = 0; i < k_num_sizes; i++)
        bench_schar(k_sizes[i]);
    for (size_t i = 0; i < k_num_sizes; i++)
        bench_short(k_sizes[i]);
    for (size_t i = 0; i < k_num_sizes; i++)
        bench_int(k_sizes[i]);
    for (size_t i = 0; i < k_num_sizes; i++)
        bench_float(k_sizes[i]);
    for (size_t i = 0; i < k_num_sizes; i++)
        bench_double(k_sizes[i]);
