...
$binFile = 'dotproduct_native.exe'
$levelTag = 'Ofast_native'
...
Write-Host "[INFO] Building with -Ofast -march=native ..."
& $compiler $srcFile '-std=c11' '-Wall' '-Wextra' '-Ofast' '-march=native' '-o' $binFile
if ($LASTEXITCODE -ne 0) {
    throw 'Compile failed for -Ofast -march=native'
}

if ($Runs -eq 1) {
    $outFile = "results_c($levelTag).csv"
    $rawCapture = "results_c($levelTag)_raw.tmp"
    ...
    Get-ChildItem -Path . -Filter "results_c($levelTag)_run*.csv" -ErrorAction SilentlyContinue | Remove-Item -Force

    $runFiles = @()
    foreach ($r in 1..$Runs) {
        $outFile = "results_c($levelTag)_run$r.csv"
        $rawCapture = "results_c($levelTag)_raw_run$r.tmp"

        Write-Host "[INFO] Running benchmark ($levelTag, run $r/$Runs) -> $outFile"
        & ".\$binFile" | Set-Content -Path $rawCapture -Encoding utf8
        Normalize-CResult -RawCapture $rawCapture -OutCsv $outFile
        Remove-Item -Force $rawCapture
        $runFiles += $outFile
    ...
    $meanFile = "results_c($levelTag)_mean.csv"
    $medianFile = "results_c($levelTag)_median.csv"
    Build-Stats -RunFiles $runFiles -MeanFile $meanFile -MedianFile $medianFile

    Copy-Item -Force "results_c($levelTag)_run$Runs.csv" "results_c($levelTag).csv"
}

Write-Host "[DONE] Generated native C files:"
Get-ChildItem -Name "results_c($levelTag)*.csv" | Sort-Object
